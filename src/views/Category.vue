      ...
      <v-container>
        ...
        <v-row>
          <v-col
            ...
            xs="12"
            lg="3"
            v-for="(CatogaryName, index) in categories"
            :key="index"
          >
            <v-btn outlined @click="fetchdata(CatogaryName)">
              {{ CatogaryName }}
            </v-btn>
          </v-col>
        </v-row>
            ...
            >
              <v-hover v-slot:default="{ hover }">
                <v-card max-width="250" max-heigth="250" align="center">
                  ...
                  <v-img :src="data.image" contain width="150" height="150"></v-img>
                  <v-card-text class="red--text" style="overflow-y: auto; height:120px" >
                       <div class="body-1">{{ data.title }}</div>
                       <v-divider light style="margin-top:15px;" />
                       <v-divider light />
                  </v-card-text>
                  <v-card-text>
                    <v-chip color="red" text--white>{{ data.price }}$</v-chip>
                  </v-card-text>
                  ...
                  <v-btn
                    ...
                    top
                  >
                    <v-icon @click="addtocart(data.id)">mdi-cart</v-icon>
                  </v-btn>
                </v-card>
              </v-hover>
            </v-col>
...
export default {
  ...
  methods: {
    fetchdata(apiId) {
      this.$store.dispatch("categories/apiCall", apiId);
    },
    // =========Get Details of signle prodcut===============
